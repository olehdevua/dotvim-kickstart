    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          --  Rebinds 0.12's default `grt` (native qf UX) to the telescope picker,
          --  matching how `grr`/`gd`/`gI` above are also routed through telescope.
...
function M.nvim_treesitter_config()
  -- New `main` branch API: no `nvim-treesitter.configs`, no `ensure_installed`
  -- /`highlight`/`indent` opts. Install parsers via `.install()` and attach
  -- highlighting with a FileType autocmd calling `vim.treesitter.start()`.
  local filetypes = {
    'json',
    'yaml',
    'toml',
  ...
  return {
    ...
    config = function()
      ...
      -- Ruby needs vim's regex highlighting alongside treesitter.
      vim.api.nvim_create_autocmd('FileType', {
        pattern = 'ruby',
        callback = function(ev)
          pcall(vim.treesitter.start, ev.buf)
